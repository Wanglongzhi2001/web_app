...
func GetCommunityPostListByTimeOrScore(p *models.ParamPostList) (postDetailList []*models.ApiPostDetail, err error) {
	// 去redis查询id列表
	...
	if len(ids) == 0 {
		zap.L().Warn("GetCommunityPostListByTimeOrScore return 0 data")
	}
	// 根据id去mysql中查找帖子详细信息，返回的数据要按照给定的id的顺序返回
	posts, err := mysql.GetPostListByIDs(ids)
	postDetailList = make([]*models.ApiPostDetail, 0, len(posts))
...
func GetPostListByTimeOrScoreNew(p *models.ParamPostList) (postDetailList []*models.ApiPostDetail, err error) {
	if p.CommunityID == 0 {
		return GetPostListByTimeOrScore(p)
	} else {
		return GetCommunityPostListByTimeOrScore(p)
	}
